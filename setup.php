...
// Delete watchlists table if it already exists
$query = 'DROP TABLE IF EXISTS watchlists';
if ($db->exec($query)===false){
	die('Query failed(9):' . $db->errorInfo()[2]);
}

// Create watchlists table
$query = 'CREATE TABLE IF NOT EXISTS watchlists (
	watchlist_id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
$titles_array = array(
	...
	array("Lost", "tv", 2004, 2010, "The survivors of a plane crash are forced to work together in order to survive on a seemingly deserted tropical island.",
...
// Array containing watchlists
// user_id, title_id
$watchlists_array = array(
	array(1, 20),
	array(1, 18),
	array(1, 13),
	array(1, 15)
...
// Inserting watchlists data
$sql = "INSERT INTO watchlists (user_id, title_id) values (?,?)";
$query = $db->prepare($sql);

foreach($watchlists_array as $entry)
{
	$query->execute($entry);
}
